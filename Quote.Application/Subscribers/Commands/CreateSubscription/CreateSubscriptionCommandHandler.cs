    ...
    ILogger<CreateSubscriptionCommandHandler> logger) : ICommandHandler<CreateSubscriptionCommand,Result<Guid>>
{
    public async Task<Result<Guid>> Handle(CreateSubscriptionCommand request, CancellationToken cancellationToken)
    {
        ...
        var telegramUser = TelegramUser.Create(request.TelegramUser,
            CaseConverter.PascalToSnakeCase(nameof(CreateSubscriberRequest.TelegramUser)), sharedViewLocalizer);
        if (telegramUser.IsFailure)
            return Result.Failure<Guid>(telegramUser.Error);
        
        if (!string.IsNullOrWhiteSpace(request.Email))
        {
            var existingEmailSubscriber = await subscriberRepository.GetByEmailAsync(request.Email, cancellationToken);
            if (existingEmailSubscriber != null)
            {
                if (!existingEmailSubscriber.IsActive)
                {
                    existingEmailSubscriber.Activate();
                    subscriberRepository.Update(existingEmailSubscriber);
                    ...
                    {
                        logger.LogError(ex, "Failed to send welcome email to {Emial}", request.Email);
                    }
                }

                return Result.Success(existingEmailSubscriber.Id);
            }

            var emailSubscriber = Subscriber.CreateWithEmail(emailResult.Value, firstName.Value, lastName.Value);
        ...
        {
            var existingTelegramSubscriber =
                await subscriberRepository.GetByTelegramChatIdAsync(request.TelegramUser.Value);
            
            if (existingTelegramSubscriber !=null)
            {
                if (!existingTelegramSubscriber.IsActive)
                {
                    existingTelegramSubscriber.Activate();
                    //await
                    subscriberRepository.Update(existingTelegramSubscriber);
                    await unitOfWork.SaveChangesAsync(cancellationToken);
                }
                return Result.Success(existingTelegramSubscriber.Id);

            }
            var telegramSubscriber = Subscriber.CreateWithTelegram(telegramUser.Value);
